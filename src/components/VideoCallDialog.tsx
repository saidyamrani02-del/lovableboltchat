...
const VideoCallDialog = ({
  ...
  const createMeteredRoom = async (appName: string, secretKey: string, roomName: string) => {
    try {
      const response = await fetch(`https://${appName}.metered.live/api/v1/room?secretKey=${secretKey}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ roomName, autoJoin: true }),
      });
      const data = await response.json();
      console.log('[METERED] Room created:', data);
      return data;
    } catch (error) {
  ...
  const joinMeteredRoom = async (appName: string, roomName: string, userName: string) => {
    try {
      ...
      meeting.on("remoteTrackStarted", (item: any) => {
        console.log('[METERED] remoteTrackStarted:', item.type);
      ...
      // Join the room
      const roomURL = `${appName}.metered.live/${roomName}`;
      console.log('[METERED] Joining room:', roomURL);
    ...
    try {
      ...
      if (!meteredSettings?.metered_app_name || !meteredSettings?.metered_secret_key) {
        toast({
          ...
          description: "Video call service is not configured. Please contact admin.",
          variant: "destructive",
        });
        onOpenChange(false);
        return;
      ...
      console.log('[CALLER] Call created:', call.id);

      // Create Metered room
      await createMeteredRoom(meteredSettings.metered_app_name, meteredSettings.metered_secret_key, roomName);

      // Store room name in call record for recipient to join
      await supabase
        .from("video_calls")
        .update({ status: `pending:${roomName}:${meteredSettings.metered_app_name}` })
        .eq("id", call.id);
            ...
            if (newStatus === "accepted") {
              ...
              try {
                await joinMeteredRoom(
                  meteredSettings.metered_app_name,
                  roomName,
  ...
  const acceptCall = async () => {
    try {
      ...
      console.log('[RECIPIENT] Room info:', { roomName, appName });
