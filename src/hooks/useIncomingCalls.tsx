...
export const useIncomingCalls = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // Check if call is recent (within last 30 seconds) - ignore stale calls
          const callTime = new Date(call.created_at).getTime();
          const now = Date.now();
          const callAge = now - callTime;
          
          if (callAge > 30000) {
            console.log('[INCOMING CALL] Call too old, ignoring:', callAge, 'ms');
            return;
          }
    ...
    // Check for any existing pending calls on mount - but only recent ones
    const checkPendingCalls = async () => {
      ...
      if (data && data.length > 0) {
        ...
        // Check if call is recent (within last 30 seconds) to avoid stale calls
        // This prevents showing missed calls when user comes online
        const callTime = new Date(call.created_at).getTime();
        const now = Date.now();
        const callAge = now - callTime;
        
        if (callAge > 30000) {
          console.log('[INCOMING CALLS] Call too old (', Math.round(callAge / 1000), 's), marking as cancelled');
        ...
        console.log('[INCOMING CALLS] Found recent pending call:', call.id, 'age:', Math.round(callAge / 1000), 's');
